    ...
    <div className="space-y-6">
          ...
          <div className="divide-y divide-slate-50">
            {thisMonthExpenses.map((exp) => (
              <div key={exp.id} className="flex items-center justify-between px-5 py-3">
                ...
                <p className="text-sm font-bold text-slate-900">{formatCLP(exp.installmentAmount)}</p>
              </div>
            ))}
          </div>
        ) : (
      ...
      {activeExpenses.length > 0 && (
        <div className="bg-white rounded-xl shadow-sm border border-slate-100">
          ...
          <div className="divide-y divide-slate-50">
            {activeExpenses.filter((e) => e.installments > 1).map((exp) => {
          ...
          <div className="divide-y divide-slate-50">
            {completedExpenses.map((exp) => (
              <div key={exp.id} className="flex items-center justify-between px-5 py-3 opacity-60">
                ...
                <div className="flex items-center gap-2">
                  <span className="text-sm text-slate-500">{formatCLP(exp.totalAmount)}</span>
                  <button
                    onClick={() => handleDelete(exp.id)}
                    className="text-slate-300 hover:text-red-500 p-1"
                  >
